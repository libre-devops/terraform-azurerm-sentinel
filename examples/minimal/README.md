...
The smallest valid call: a fresh Log Analytics workspace onboarded to Microsoft Sentinel, nothing
else. The environment comes from the Terraform workspace (`terraform.workspace`), not a variable. Run it with `just e2e minimal`, which applies the stack then always destroys it.
...
```hcl
# Minimal call: a fresh Log Analytics workspace onboarded to Microsoft Sentinel. Sentinel starts a
# 31 day free trial per workspace, and this stack is applied then destroyed in one CI run, so
# nothing lingers to bill.
locals {
